<?php

@session_start();

include "koneksi.php";

if (@$_SESSION['admin'] || @$_SESSION['accounting']) {
?>

    <!DOCTYPE html>
    <html>

    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <title></title>
        <link rel="stylesheet" href="./bootstrap/css/bootstrap.min.css">
        <link rel="stylesheet" href="./css/bootstrap-datetimepicker.min.css">
        <style>
    .white-footer {
        background-color: white;
        padding: 20px; /* Adjust the padding as needed */
        text-align: center;
        position: fixed;
        left: 0;
        bottom: 0;
        width: 100%;
    }
    body {
        margin: 0; /* Remove default body margin */
        padding-bottom: 60px; /* Adjust based on the height of your footer */
        background-image: url('img/main1.jpg');
        background-size: cover;
        background-repeat: no-repeat;
        background-attachment: fixed;
    }
</style>

    </head>

    <body>
        <div class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            <div class="container">
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
                        <span class="sr-only">Toggle navigation</span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                    </button>
                </div>
                <div class="navbar-collapse collapse">
                    <ul class="nav navbar-nav">
                        <li class="dropdown active">
                            <a href="index.php" class="dropdown" data-toggle="dropdown">Akuntansi App</a>
                        </li>
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">Jurnal <span class="caret"></span></a>
                            <ul class="dropdown-menu" role="menu">
                                <li><a href="jurnalpembelian.php">Jurnal Pembelian</a></li>
                                <li><a href="jurnalpenjualan.php">Jurnal Penjualan</a></li>
                                <li><a href="jurnalpenggajian.php">Jurnal Penggajian</a></li>
                                <li><a href="jurnalkas.php">Jurnal Penerimaan Kas</a></li>
                                <li><a href="jurnalpengeluaran.php">Jurnal Pengeluaran Kas</a></li>
                            </ul>
                        </li>
                        <li class="">
                            <a href="jurnalUmum.php" class="dropdown-toggle">Jurnal Umum </a>
                        </li>
                        <li class="">
                            <a href="bukuBesar.php" class="dropdown-toggle">Buku Besar </a>
                        </li>
                        <li class="">
                            <a href="logout.php" class="dropdown-toggle">Keluar </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="panel panel-default" style="padding-top: 100px">
            <div class="panel-heading">
                <h2>
                    <marquee>Selamat Datang | post by <a href="" title="EFDS CORPS" target="_blank">EFDS CORPS</a></marquee>
                </h2>
            </div>


            <div class="footer">
    <?php
    // Display current day, date, and time
    ?>
</div>

<!-- Add a new div for the white background footer -->
<div class="white-footer">
    <!-- Content for the white background footer goes here -->
</div>

<script type="text/javascript" src="./jquery/jquery-1.8.3.min.js" charset="UTF-8"></script>
<script type="text/javascript" src="./bootstrap/js/bootstrap.min.js"></script>
<script>
    // JavaScript code to update the time every second
    setInterval(function () {
        var now = new Date();
        var hours = now.getHours();
        var minutes = now.getMinutes();
        var seconds = now.getSeconds();
        document.querySelector('.white-footer').innerHTML = "Hari ini adalah " + now.toLocaleDateString('en-US', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
        }) + " | Pukul " + hours + ":" + minutes + ":" + seconds;
    }, 1000);
</script>
    </body>

    </html>
<?php
} else {
    header("location:index.php");
}
?>